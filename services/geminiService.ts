...
export const findProductAlternatives = async (
  ...
  const prompt = `Find 3 excellent alternative products for: "${sanitizeForPrompt(brand)} ${sanitizeForPrompt(itemName)}" (${sanitizeForPrompt(category)}).
...
export const askGrowAssistant = async (
  ...
  Managed by: ${sanitizeForPrompt(settings.userName)} (${sanitizeForPrompt(String(settings.experienceLevel))} grower).
  Date: ${currentDate}.
...
export const analyzeGrowData = async (
  ...
  const logSummary = logs.slice(0, 50).map(l => `${l.date}: ${l.action} ${l.amount}${l.unit} of ${sanitizeForPrompt(l.itemName)} (${sanitizeForPrompt(l.category)})`).join('\n');

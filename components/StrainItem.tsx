...
import { Pencil, Trash2, ExternalLink, Clock, Flower, StickyNote, Sprout } from 'lucide-react';
...
export const StrainItem: React.FC<StrainItemProps> = memo(({ strain, onEdit, onDelete, onPopSeeds }) => {
  return (
      ...
      <div>
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="flex opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={() => onEdit(strain)} className="text-gray-400 hover:text-blue-500 transition-colors mr-2" aria-label="Edit strain"><Pencil size={16} /></button>
            <button onClick={() => onDelete(strain.id)} className="text-gray-400 hover:text-red-500 transition-colors" aria-label="Delete strain"><Trash2 size={16} /></button>
          </div>
        </div>
